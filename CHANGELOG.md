...
A large pass covering an Android correctness bug, dictionary/reading performance, and a broad set of reading-experience features: bookmarks, in-book/library search, swipe page-turns, Night mode, and inline editing throughout. Speed Reader is hidden (not removed) for now.

- **Fixed: "no definition found" for every word on Android.** Root cause: the bundled AraMorph dictionary data was fetched over the network at startup (`fetch('/dictionary-data/...')`), and on at least one real Android/Capacitor WebView that fetch silently resolved with only a small corrupted fragment of each file — no error, just wrong data, which is worse than a clean failure. Diagnosed by adding on-device diagnostics (Settings → Dictionaries → "Test dictionary lookup", showing per-table entry counts) and confirming the same files read via the browser's File API (manual upload) always came through intact. Fixed by embedding the six data files directly into the JS bundle at build time (`vite.config.ts`'s `bundledDictDataPlugin`, a virtual module) instead of fetching them — removes the network/asset-serving layer that was going wrong, on every platform, not just Android.
- **Dictionary engine moved to a Web Worker** (new `aramorph.worker.ts`). Building the AraMorph lookup tables (~136k `dictstems` lines) is real CPU work that used to run on the main thread at every startup and on every "Reset to default"/custom upload, competing with rendering the UI. `AramorphDictionaryProvider` is now a thin `postMessage` proxy with the same public API. Side effect: this also code-split the embedded dictionary data out of the main bundle (it dropped from ~4.7MB back to ~700-800KB), so the app shell paints before the dictionary finishes loading instead of after.
- **Vocabulary Levels now runs on a personal, hand-curated ~5,300-word frequency list** ("The List") instead of the CAMeL Arabic Frequency Lists (an 11.4M-word corpus that had to be fetched at ~65MB compressed and parsed into a ~800MB in-memory `Map`, taking 10-15 seconds per session even with the parse loop yielding to the event loop so the UI didn't fully freeze). The new list is embedded at build time the same way the dictionary is, parses in well under a millisecond, and needs none of the old opt-in/progress-reporting machinery, which was removed along with the CAMeL dataset's fetch/caching code and attribution. Rarity tier cutoffs (beginner/intermediate/advanced) rescaled to the new list's size — the old cutoffs were sized for the 11.4M-word corpus and would have made "advanced" unreachable for any word actually in the new list. Six data-entry errors found and fixed while converting the source document (missing separators, a corrupted gloss, one entry with no gloss at all, a duplicate).
- **Real citation/dictionary forms for lookups and saved vocabulary**, not just the root. AraMorph's own data already tags every stem group with a lemma marker (e.g. `;; kAtab_1` for كاتَبَ, Form III of ك-ت-ب) distinct from the root marker — previously parsed as a comment and discarded. `engine.ts`'s table parser now captures it; the dictionary popup shows it as "Dictionary form" next to Root, and saved vocabulary cards get the real value instead of the old placeholder (`lemma: r.word`, which just echoed back the surface form).
- **Focus mode actually hides everything now.** It previously only faded the topbar; the footer and the app-level sidebar were untouched. All three now fade/collapse together, and reveal is tiered by deliberateness: a plain tap/click brings back the topbar and footer (scroll and page-turn drags don't count, since they don't fire a click), but the sidebar only reveals via a left-edge drag (≥48px, starting within 24px of the edge) — bringing back a whole panel is a bigger interruption than a faded topbar, so it needed more friction than "any touch."
- **New "Aa" quick-settings popover** in the reader topbar (font size, theme swatches, paged/scrolling), modeled on Apple Books' own reading-time controls — separate from the full Settings panel, which stays for everything reached less often (dictionaries, backup, Anki, touch gestures).
- **Multi-definition vocabulary saving.** A word with more than one genuinely distinct dictionary entry (different roots/senses, not just different senses of the same headword) can now be saved as one combined card (the popup's main "Save Vocabulary" button) or as individual per-entry cards (a small "+" next to each entry). Saved cards can be switched between "all definitions" and one specific entry, or split into separate cards after the fact, from a dropdown in the Vocabulary tab.
- **Speed Reader hidden from navigation** (`NavBar.tsx`'s `SPEED_READER_ENABLED` flag) — fully implemented and still routed in `App.tsx`, just not linked to from the nav bar for now.
- **Vocabulary save/un-save is a real toggle in the dictionary popup** — "Save Vocabulary" / "✓ Vocabulary", pressing it again removes every card for that word without needing to open Edit. Saved words in the reading text get a font-colour-only muted red (reusing the app's existing `--danger` token, no new colour), applied live and re-tinted immediately on theme switch, not just on next render.
- **Compact vocabulary edit modal** (`VocabularyEditModal.tsx`) over the reading page — word/meaning/context fields, word-level press-and-drag selection on the original context sentence (tap or drag a range of words to become the new context), Restore original / Delete context, Cancel / Save.
- **Review flashcards use inline editing**, not a popup — tap the meaning or context sentence directly on the flipped card to edit it in place (pencil icon on hover), Enter/blur commits, Escape cancels. Reuses `vocabularyService.updateVocabularyItem`, the same call the Vocabulary tab's own inline editor already used.
- **Bookmarks** — new feature, any number per book, independent of highlights and of the automatic per-book reading position. Own icon (outline vs. filled ribbon, not colour-only), a panel off the reader topbar, individually removable. Location reference started as a percent label and was upgraded to a real "Page N of Total" once epub.js's `locations` index is generated for the book (`EpubService.generateLocations`/`getPageLabel`) — that generation is slow (walks the entire book's text) so it runs once in the background well after the book is already readable, and is cached (`bookLocations` table) so it never repeats for the same book.
- **Dictionary popup repositions itself to always stay fully on-screen**, replacing the old fixed-height-guess clamping with real measurement (renders invisibly, measures its actual box, then places it) — plus a "Dictionary popup size" 70–150% slider in Settings, applied independently of the positioning logic via `transform: scale()`.
- **Reading position** (CFI + percent + chapter, saved on every navigation, restored on reopen) was already fully implemented from earlier work — verified rather than rebuilt.
- **Diacritic-insensitive, hamza-normalized in-book and library search.** New `normalizeForSearch()` (deliberately separate from the `normalize()` dictionary lookups use, which already gets correct hamza-variant matching for free from the bundled data) strips diacritics/tatweel and folds أ/إ/آ/ٱ→ا, tracking an index map back to the original text so matches still produce correct CFIs and un-normalized excerpts. `EpubService.search()` rewritten around it instead of epub.js's plain-substring `Section.find()`.
- **Search is a real overlay** now (dimmed backdrop, centered panel, Esc closes it on desktop) instead of a docked sidebar, with result count + ↑/↓ prev/next navigation, live search (debounced, Settings toggle), and search history (Settings toggle, recent-search chips) — plus scope (This page / This book / Library — Library opens each book in the library in a hidden container, searches it, tears it down, and aggregates results, reusing the same `search()`) and mode (Exact phrase / Any word) options, and match-quality tiering (exact literal match ranked above one that only lines up after normalization, above a loose word-mode match). A Library-scope result for a different book than the one currently open hands off through a new `onOpenBookAt` (threaded through `App.tsx`) that switches books and opens at that precise location.
- **Swipe page-turn navigation.** epub.js's own `rendition.direction()` now actually drives page-turn/spread behavior (previously RTL was only forced via CSS text-direction, not epub.js's page-turn math) — a new "Page direction" Automatic/RTL/LTR setting resolves against the book's own declared direction (falling back to RTL) or explicitly overrides it. A horizontal swipe (≥60px, predominantly horizontal, suppressed if the gesture produced a real text selection) maps to next()/previous() based on the book's actual direction, via a touch listener kept fully independent from the existing word-tap/hold gesture system so the two never compete for the same touch.
- **Night mode** — the existing dark theme already met the "muted, not generic black" bar, so this was mostly wiring: relabeled "Dark" to "Night" in the UI (stored value unchanged, no migration needed), added a `system` option that follows `prefers-color-scheme` live, including a theme change made while the app is already open.
- All of the above settings (Page direction, Live search, Search history, Dictionary popup size, System theme) integrated into the existing Settings panel — no parallel settings system.
