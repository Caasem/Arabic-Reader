...
export function ActivityCalendar({
  ...
  const { weeks, max, totalDays } = useMemo(() => {
    ...
    // Align the grid so the last column ends on today's weekday, starting on a Sunday.
    const firstDay = addDays(today, -(WEEKS * 7 - 1));
    const start = addDays(firstDay, -new Date(firstDay).getDay());

    const cells: { date: string; ms: number; inRange: boolean }[] = [];
    let max = 0;
    let totalDays = 0;
    for (let i = 0; i < WEEKS * 7; i++) {
      // Calendar-day steps: adding 24h would drift a day across DST changes.
      const t = addDays(start, i);
      const key = dayKey(t);
      const ms = byDate.get(key)?.activeDurationMs ?? 0;
      if (ms > 0) totalDays++;
      max = Math.max(max, ms);
      cells.push({ date: key, ms, inRange: t <= today });
    }
